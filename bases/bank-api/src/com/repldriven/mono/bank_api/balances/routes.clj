(ns com.repldriven.mono.bank-api.balances.routes
  (:require
    [com.repldriven.mono.bank-api.balances.handlers
     :as handlers]
    [com.repldriven.mono.bank-api.balances.queries
     :as queries]
    [com.repldriven.mono.bank-api.balances.examples
     :refer [BalanceNotFound]]
  ...
  [["/cash-accounts/{account-id}/balances"
    ...
    ["/{balance-type}/{currency}/{balance-status}"
     {:get {:summary "Retrieve a balance"
            ...
            :parameters
            {:path {:balance-type string?
                    :currency string?
                    :balance-status string?}}
            :responses
            {200 {:body [:ref "Balance"]}
   ...
   ["/cash-account-products/{product-id}/versions/{version-id}/balance-products"
    ...
    [""
     {:get
      {:summary "Retrieve balance products for a version"
       :openapi
       {:operationId "RetrieveBalanceProducts"}
       :responses
       {200 {:body [:ref "BalanceProductList"]}}
