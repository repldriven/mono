...
(defn list-balances
  ...
  (let [{:keys [record-db record-store]} request
        account-id (get-in request [:parameters :path
                                    :account-id])
        result (balances/get-balances
                {:record-db record-db
    ...
    (if (error/anomaly? result)
      ...
      {:status 200
       :body {:balances (mapv format-balance result)}})))

(defn get-balance
  [request]
  (let [{:keys [record-db record-store]} request
        {:keys [account-id balance-type currency
                balance-status]}
        (get-in request [:parameters :path])
        result (balances/get-balance
                {:record-db record-db
                 :record-store record-store}
                account-id
                (keyword balance-type)
                currency
                (keyword balance-status))]
    (cond
