...
A system that answers callers cannot afford unstructured or silent
failures. Exceptions in Clojure (and the Java libraries that underlie
most of the stack) are the default failure mechanism, but they have
well-known weaknesses for a system of this kind:
...
A system also has more than one *kind* of failure to surface.
...
missing prerequisite, a duplicate, a state that does not allow the
request), which is not the same as "the caller has no business
asking" (no credential, another owner's resource). Collapsing all
three into one notion of error makes the API layer's job harder and
obscures real bugs.

We need a single convention that handles all this.
...
2. Three anomaly kinds, each a distinct nom tag, with matching
   ...
   - `:rejection/anomaly` — the system worked correctly and decided
     not to perform the requested operation (missing prerequisite,
     duplicate, conflict, a state that does not allow it). Constructor:
     `error/reject`.
   - `:unauthorized/anomaly` — the caller is not permitted to make
     this call at all. Constructor: `error/unauthorized`.

   This split lets the API layer map cleanly to HTTP status families:
   errors become 5xx, rejections become 4xx, unauthorized becomes
   401/403. The command envelope carries the same split — a
   `CommandResponse` status is accepted, rejected or failed.
...
(defn article-for
  ...
  (error/let-nom> [article (store/find-article store slug)
                   _       (when-not article
                             (error/reject :realworld/article-not-found
                                           {:message "no such article"
                                            :slug slug}))
                   author  (store/profile store (:author-id article)
                                          viewer-id)]
    (assoc article :author author)))
```

This is the most load-bearing convention in the bricks. Every
interface boundary, every component and every test relies on it, and
a workspace built on them inherits it whole.

### Naming a category

A category is an internal diagnostic until an API surfaces it to a
client. The `server` brick's problem bodies are RFC 9457-shaped, and
RFC 9457 defines `type` as identifying the *problem*, not its origin —
so an API that puts the category in `type` has made it a statement of
the problem. The line is whether anyone outside the process can act on
it.

**A rejection always names the problem.** A 4xx exists to tell a
caller what they got wrong so they can fix it and retry —
`:realworld/article-not-found`, `:realworld/username-taken`,
`:keycloak/client-not-found`. A rejection named for its call site
would be useless to the caller and would defeat the mapping from kind
to status, which reads the category: `realworld-domain` keeps that
mapping as one table so the read path and the write path cannot
disagree.
...
so. A store's contention and timeout failures are the usual case: its
own client distinguishes them, a caller branches on them, and both map
to 503 because both mean back off and retry — they differ in what they
say about the store, not in what the caller should do. The distinction
rides `type`, which is the case for naming the problem there rather
than splitting the status.
...
illustrative — `http-client` names by call site, as do `:json/parse`,
`:kafka/producer-send` and the rest. Both halves name the same brick
so the example isolates the naming choice.
...
- Test code asserts "this returns an anomaly" without try/catch
  scaffolding; `test-system`'s `nom-test>` macro composes the same way
  `let-nom>` does but fails fast on any anomaly.
